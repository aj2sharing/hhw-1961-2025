...
OUT_DIR = Path(r"J:\EP_events_1961-2025")
...
EVENT_SCHEMA = pa.schema([
    ...
    ("peak_date", pa.date32()),
    ("peak_precip", pa.float32()),
    ("peak_z_precip", pa.float32()),
])
...
def identify(overwrite=False, threshold_overwrite=False):
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    ANNUAL_DIR.mkdir(parents=True, exist_ok=True)

    compute_precip_p90_file(overwrite=threshold_overwrite)
    threshold, _, _ = load_precip_p90()
    total_events = 0

    for year in YEARS:
        out_file = ANNUAL_DIR / f"EP_Events_{year}.parquet"
        if out_file.exists() and not overwrite:
            print(f"[ep-events] {year} exists, skip", flush=True)
            continue
        writer, tmp_file = open_parquet_writer(out_file, EVENT_SCHEMA)

        precip_ext, time_ext, precip_handles, cur_len, lon, lat = concat_year_with_padding(
            year,
            open_precip,
            pad_before=DEFAULT_PAD_DAYS,
            pad_after=DEFAULT_PAD_DAYS,
        )
        zp_ext, _, zp_handles, _, _, _ = concat_year_with_padding(
            year,
            open_z_precip,
            pad_before=DEFAULT_PAD_DAYS,
            pad_after=DEFAULT_PAD_DAYS,
        ...
        try:
            exceed_ext = precip_ext > threshold[None, :, :]
            ...
            for j, i, starts, ends in iter_event_bounds(exceed_ext, year_start, year_end, lon.size):
                for start_idx, end_idx in zip(starts, ends):
                    ...
                    precip_segment = precip_ext[start_idx:end_idx + 1, j, i]
                    if np.all(np.isnan(precip_segment)):
                        continue
                    peak_offset = int(np.nanargmax(precip_segment))
                    peak_idx = start_idx + peak_offset
                    peak_precip = float(precip_ext[peak_idx, j, i])
                    peak_z_precip = float(zp_ext[peak_idx, j, i])
                    if not np.isfinite(peak_precip) or not np.isfinite(peak_z_precip):
                        continue
            ...
            print(f"[ep-events] {year} events: {year_events}", flush=True)
            finalize_parquet_writer(writer, tmp_file, out_file)
        finally:
            for ds in precip_handles + zp_handles:
                ds.close()

    print(f"[ep-events] total events: {total_events}", flush=True)
    print(f"[ep-events] saved annual files to {ANNUAL_DIR}", flush=True)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--overwrite", action="store_true")
    parser.add_argument("--overwrite-threshold", action="store_true")
    args = parser.parse_args()
    identify(overwrite=args.overwrite, threshold_overwrite=args.overwrite_threshold)
