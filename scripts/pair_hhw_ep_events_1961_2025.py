...
OUT_DIR = Path(r"J:\HHW_EP_1961-2025_vectors_v2")
MAX_LAG = 7
...
def select_best_ep_candidate(ep_group, hhw_peak: np.datetime64):
    peak_dates = ep_group["peak_date"]
    left = np.searchsorted(peak_dates, hhw_peak, side="left")
    right = np.searchsorted(peak_dates, hhw_peak + np.timedelta64(MAX_LAG, "D"), side="right")
    if right <= left:
        return None

    zvals = ep_group["peak_z_precip"][left:right]
    if zvals.size == 0:
        return None
    ...
    rel_idx = int(np.flatnonzero(zvals == best_z)[0])
    idx = left + rel_idx
    return {
...
def identify(overwrite=False):
    ...
    event_parquet = OUT_DIR / "HHW_EP_Event_List_1961-2025_vectors.parquet"
    grid_nc = OUT_DIR / "HHW_EP_Gridded_Metrics_1961-2025_vectors.nc"
    ...
    for year in YEARS:
        ...
        for key, hhw_group in hhw_groups.items():
            ...
            for idx_evt in range(hhw_group["peak_date"].size):
                ...
                best = select_best_ep_candidate(ep_group, hhw_peak)
                if best is None:
                    continue
                ...
                if not all(np.isfinite(v) for v in [z_twb_hhw, z_precip_ep, z_precip_hhw, z_twb_ep]):
                    continue

                lag = int((ep_peak - hhw_peak).astype(int))
                dx = z_twb_ep - z_twb_hhw
                dy = z_precip_ep - z_precip_hhw
                cooling = -dx
                distance = float(np.sqrt(dx * dx + dy * dy))
                speed = distance / (lag + 1.0)
                angle = float(np.arctan2(dy, cooling))
                wetting = dy
                ...
                vals = [lag, distance, speed, cooling, angle, wetting]
        ...
        print(f"[pair-hhw-ep] {year} events: {year_events}", flush=True)
    ...
    write_grid_metrics(
        ...
        counts,
        sums,
        ["lag", "distance", "speed", "cooling", "angle_rad", "wetting"],
        "HHW-EP vector metrics, 1961-2025",
        "Paired from independent HHW and EP annual event tables; HHW peak precedes EP peak by 0..7 days; overlap allowed.",
    )
    print(f"[pair-hhw-ep] total events: {total_events}", flush=True)
    print(f"[pair-hhw-ep] saved {event_parquet}", flush=True)
    print(f"[pair-hhw-ep] saved {grid_nc}", flush=True)
